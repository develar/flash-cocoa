...
//			}
	...
	{
		var loader:FlexModuleLoader = FlexModuleLoader(event.loader);
		if (loader.moduleInfo.category == applicationCategory)
		{
			...
			{
				dispatchContextEvent(new LoadModuleEvent(initialModuleLoadedEventType, event.moduleInfo));
			}
			currentApplicationWideModule = event.moduleInfo;
			// @todo установить новый skin в BrowserManager
		}
		else
		{
			dispatchContextEvent(new LoadModuleEvent(moduleLoadedEventType, event.moduleInfo));
		}
